...
class SiteOrigin_Panels_Widget_Hero extends SiteOrigin_Widget_Base_Slider {
	...
	function __construct() {
		parent::__construct(
			'sow-hero',
			__('Hero Widget', 'siteorigin-widgets'),
			array(
	...
	function initialize(){
		if( !class_exists('SiteOrigin_Panels_Widget_Button') ) {
			...
			include plugin_dir_path( SOW_BUNDLE_BASE_FILE ) . 'widgets/so-button-widget/button.php';
			siteorigin_widget_register( 'button', plugin_dir_path( SOW_BUNDLE_BASE_FILE ) . 'widgets/so-button-widget/button.php' );
		}
	...
	function process_content( $content, $frame ) {
		...
		foreach( $frame['buttons'] as $button ) {
			$this->sub_widget('SiteOrigin_Panels_Widget_Button', array(), $button['button']);
		}
